...
def test_map_response_recovers_textual_tool_call():
    bedrock_response = {
        "responseId": "r-textual",
        "output": [
            {
                "role": "assistant",
                "content": [
    ...
    payload = {"model": "eu.amazon.nova-lite-v1:0", "messages": [{"role": "user", "content": "włącz biuro l1"}]}

    result = handler.map_response("eu.amazon.nova-lite-v1:0", payload, bedrock_response)

    assert len(result["tool_calls"]) == 1
    tool = result["tool_calls"][0]
    assert tool["function"]["name"] == "execute_services"
    args = json.loads(tool["function"]["arguments"])
    assert args["list"][0]["service_data"]["entity_id"] == "switch.biuro_l1"
