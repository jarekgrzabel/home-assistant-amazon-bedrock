...
TOOL_CALL_PATTERN = re.compile(r"\[tool_call\s+([a-zA-Z0-9_]+)\s+args=(\{.*\})\]", re.DOTALL)
...
def infer_cover_intent_from_text(text: str) -> Dict[str, Any]:
    if not text:
        return {}
    lowered = text.lower()
    result: Dict[str, Any] = {}
    ...
    if any(kw in lowered for kw in ["pochyl", "lamel", "tilt", "lamele"]):
        result["kind"] = "tilt"
    elif any(kw in lowered for kw in ["ustaw", "podnie", "otwórz", "otworz", "opuść", "opusc", "zamknij"]):
        result["kind"] = "position"

    if any(kw in lowered for kw in ["całkiem", "calkiem", "całkowicie", "calowicie", "fully"]):
        if any(kw in lowered for kw in ["zamkn", "opuść", "opusc", "close"]):
            result["percent"] = 0
        else:
            result["percent"] = 100

    if "połow" in lowered or "polow" in lowered:
        result.setdefault("percent", 50)

    return result


def sanitize_tool_arguments(arguments: Any, user_text: str) -> Any:
    if isinstance(arguments, dict):
        actions = arguments.get("list")
        if isinstance(actions, list):
            cover_context = infer_cover_intent_from_text(user_text)
            for action in actions:
                ...
                if domain == "cover" and isinstance(service_data, dict):
                    if cover_context.get("kind") == "tilt" and service != "set_cover_tilt_position":
                        action["service"] = "set_cover_tilt_position"
                        service = action["service"]

                    required_key = None
                    if service == "set_cover_position":
                        required_key = "position"
                    elif service == "set_cover_tilt_position":
                        required_key = "tilt_position"

                    if required_key:
                        if required_key not in service_data:
                            if "percent" in cover_context:
                                service_data[required_key] = cover_context["percent"]
                        if required_key not in service_data:
                            # default to 100 for open commands (open/podnieś) or 0 for close
                            if cover_context.get("kind") == "position":
                                if any(kw in user_text.lower() for kw in ["zamkn", "opuść", "opusc", "close"]):
                                    service_data[required_key] = 0
                                elif any(kw in user_text.lower() for kw in ["otw", "podnie", "raise", "open"]):
                                    service_data[required_key] = 100
                        if required_key not in service_data:
                            # Remove invalid action if still missing value
                            action["_invalid"] = True
                    elif service in {"open_cover", "close_cover"} and "position" in cover_context:
                        # Prefer explicit percentage over open/close
                        action["service"] = "set_cover_position"
                        service_data["position"] = cover_context["percent"]

                if not action.get("service") or action.get("_invalid"):
                    LOGGER.info("Dropping invalid action from toolUse: %s", action)
                    action["_remove"] = True

            # filter invalid actions
            arguments["list"] = [action for action in actions if not action.get("_remove")]
    return arguments
...
def parse_textual_tool_calls(text: str) -> List[Dict[str, Any]]:
    ...
    for match in TOOL_CALL_PATTERN.finditer(text):
        ...
        try:
            ...
            LOGGER.warning("Failed to parse textual tool_call args: %s", redact_pii(args_raw))
            continue
        parsed_calls.append(
            {
                "id": f"textual-tool-{uuid.uuid4()}",
                "type": "function",
                "function": {
                    "name": name,
...
def map_tool_calls(outputs: List[Dict[str, Any]], user_text: str) -> List[Dict[str, Any]]:
    ...
    for output in outputs:
        ...
        for content in content_list:
            ...
            arguments = sanitize_tool_arguments(arguments, user_text)
            tool_calls.append(
                {
...
def map_response(model_id: str, payload: Dict[str, Any], response: Dict[str, Any], mode: str = "responses") -> Dict[str, Any]:
    ...
    last_user_text = get_last_user_text(payload)
    tool_calls = map_tool_calls(outputs, last_user_text)
    ...
    if not tool_calls:
        textual_calls = parse_textual_tool_calls(output_text)
        if textual_calls:
            LOGGER.info("Recovered %d textual tool_call(s) from assistant output", len(textual_calls))
            tool_calls = textual_calls
